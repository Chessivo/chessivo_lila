...
<div id="zenzone">
  ...
  private def dasher(me: lila.user.User) =
    div(cls := "dasher")(
      a(id := "user_tag", cls := "toggle link")(me.username),
...
"""
  ...
  private def botImage =
    img(
      src   := assetUrl("images/icons/chessivo.png"),
      title := "Robot chess",
      style :=
        "display:inline;width:48px;height:48px;vertical-align:top;margin-right:5px;vertical-align:text-top"
    )
  ...
  def apply(
    ...
    frag(
      ...
      htmlTag(using ctx.lang)(
        ...
        div(cls := "footer")("Lichess  is ", a(href := "https://github.com/ornicar/lila")("free."), " Chessivo.com is using a ", a(href := "https://github.com/ornicar/lila")("fork"), " to display the analysis.")
      )
    )
...
<label for="tn-tg" class="hbg"><span class="hbg__in"></span></label>"""
    ...
    def apply(zenable: Boolean)(using ctx: Context) = {
      def renderDiv(hrefLink: String, ctx: Context) =
        div(cls := "site-title-nav")(
          !ctx.isAppealUser option topnavToggle,
          h1(cls := "site-title")(
            botImage,
            a(href := hrefLink)(siteNameFrag)
          ),
          ctx.blind option h2("Navigation"),
        )
      header(id := "top")(
        ctx.me match {
          case Some(user) => renderDiv(routes.User.show(user.username).url,ctx)
          case None => renderDiv("https://chessivo.com/login",ctx)
        },
        div(cls := "site-buttons")(
          ...
          ctx.me map { me =>
            ...
            !ctx.pageData.error option anonDasher
          }
