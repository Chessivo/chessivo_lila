...
object widgets:
  ...
  def apply(
    ...
    games map { g =>
      ...
      val povColor = g.metadata.aiside match {
        case 1 => chess.Color.White
        case 2 => chess.Color.Black
        case _ => firstPlayer.color
      }
      st.article(cls := "game-row paginated")(
        a(cls := "game-row__overlay", href := gameLink(g, povColor, ownerLink)),
        div(cls := "game-row__board")(
          views.html.board.bits.mini(Pov(g, povColor))(span)
        ),
        div(cls := "game-row__infos")(
          ...
          (g.metadata.analysed && g.metadata.aianalysed) option
            div(cls := "metadata text", style := "color: hsl(307, 80%, 59%);", dataIcon := licon.Microscope)("AI Analysis available"),
          (g.metadata.analysed && !g.metadata.aianalysed) option
            div(cls := "metadata text", style := "color: hsl(307, 80%, 59%);", dataIcon := licon.Reload)("Waiting for AI Analysis... Please check back later."),
          //g.pgnImport.flatMap(_.user).map { user =>
