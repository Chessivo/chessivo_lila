...
import lila.user.{ Holder, TotpSecret }
...
final class Auth(
  ...
  def authenticate = OpenBody:
    Unauthorized(jsonError("This account is closed.")).toFuccess
  ...
  def signupPost = OpenBody:
    fuccess(BadRequest(jsonError("Signup disabled")))
  ...
  def passwordResetApply =
    OpenBody:
      fuccess(BadRequest(jsonError("Password reset disabled")))
  ...
  def magicLinkLogin(token: String) = Open:
    if ctx.isAuth
    then {
      lila.log("prismic").error(s"Already AUTHed, redirecting")
      ctx.me match {
        case Some(user) => Redirect(routes.User.show(user.username)).toFuccess
        case None => {
          lila.log("prismic").error(s"NO USR")
          Redirect(routes.Lobby.home).toFuccess
        }
      }
    }
    else
    {
      // Parse token
      //lila.log("prismic").error(s"MLL ${token} SEC ${env.security.magicLink.mysecret}")
      val key = env.security.magicLink.mysecret
      // password reset
      val mytoken = token.trim.stripPrefix("token=")
      //lila.log("prismic").error(s"TOKEN ${mytoken}")
      val result = JwtJson4s.decodeJson(mytoken, key, Seq(JwtAlgorithm.HS512)) match {
        case Success(claim: JObject) =>
          // Now we return the claim object in the success case
          //lila.log("prismic").error(s"Claim: $claim")
          Some(claim)
        case Failure(e) =>
          lila.log("prismic").error(s"Failed to decode token: $e")
          None
      }
      implicit val formats = DefaultFormats // This is needed for extract method
      result match {
        case Some(claim) =>
          @nowarn val userId = (claim \ "data" \ "userId").extract[String] // Suppress deprecation warning
          lila.log("prismic").error(s"TOKEN IS VALID, userId: ${userId}")
          val maybeUserFuture = env.user.repo byId UserStr(userId)

          maybeUserFuture.flatMap { maybeUser =>
            maybeUser match {
              case Some(usr: lila.user.User) =>
                lila.log("prismic").error(s"User found, user: ${usr}")
                val redirectTo = routes.User.show(userId).url
                authenticateUser(usr, remember = true, result = Some(_ => Redirect(redirectTo))) >>-
                  lila.mon.user.auth.magicLinkConfirm("success").increment().unit
                //Future.successful(Redirect(routes.User.gamesAll(userId)))
              case None =>
                lila.log("prismic").error("User not found")
                Future.successful(Redirect(routes.Lobby.home))
            }
          }.recover { case e =>
            lila.log("prismic").error("Failed to fetch user", e)
            Redirect(routes.Lobby.home)
          }
        case None => Future.successful(Redirect(routes.Lobby.home))
      }
