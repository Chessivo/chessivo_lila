...
final class Opening(env: Env) extends LilaController(env):
  ...
  def wikiWrite(key: String, moves: String) = SecureBody(_.OpeningWiki) {
    ctx ?=> me
    =>
    env.opening.api
      ...
      .flatMap { op =>
        lila.log("prismic").error(s"wikiWrite. $moves") // Your log output
        val redirect = Redirect(routes.Opening.byKeyAndMoves(key, moves))
        lila.opening.OpeningWiki.form
          .bindFromRequest()
          .fold(
            _ => redirect.toFuccess,
            text => env.opening.wiki.write(op.get, text, me.user) inject redirect
          )
      }
  }
